				...
				if(pProperties[i].szProperty && (pProperties[i].dwType > 0) && (pProperties[i].dwType < 20))
				{
					...
					if(szPropertyName)
					{
						...
						if(bToDisplay)
						{
							...
							{
							case 1:
								kprintf(L"[ dword ] %u (0x%08x)", (ULONG_PTR) pProperties[i].pvData, (ULONG_PTR) pProperties[i].pvData);
								break;
							...
							case 6:
								kprintf(L"[protect] ");
								if(pProperties[i].pvData && pProperties[i].unkp2)
								{
									aDataBuffer.address = (PBYTE) LocalAlloc(LPTR, (SIZE_T) pProperties[i].unkp2);

									if(aDataBuffer.address)
									{
										aProcess.address = pProperties[i].pvData;
										if(kull_m_memory_copy(&aDataBuffer, &aProcess, (SIZE_T) pProperties[i].unkp2))
										{
											if(pProperties[i].dwFlags & 0x800)
											{
												if(kull_m_crypto_remote_CryptUnprotectMemory(aProcess.hMemory, aDataBuffer.address, (SIZE_T) pProperties[i].unkp2 & 0xffffffff, CRYPTPROTECTMEMORY_SAME_PROCESS))
												{
											...
											{
												kull_m_string_wprintf_hex(aDataBuffer.address, (SIZE_T) pProperties[i].unkp2 & 0xffffffff, 0);
											}
										}
